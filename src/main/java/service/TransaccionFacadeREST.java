...
public class TransaccionFacadeREST extends AbstractFacade<Transaccion> {
    ...
    public void addTransacion(String json){
        ...
        t.setFecha(new Date(Integer.parseInt(jo.get("fecha").toString().replaceAll("\"", "").split("-")[0])-1901, Integer.parseInt(jo.get("fecha").toString().replaceAll("\"", "").split("-")[1]), Integer.parseInt(jo.get("fecha").toString().replaceAll("\"", "").split("-")[2])));
